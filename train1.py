...
from models7 import VAE_GAN,Discriminator
# nvidia-smi -l 0.2
...
if __name__=='__main__':
    ...
    for epoch in range(epochs):
        ...
        for i, (data,_) in enumerate(train_loader, 0):
            ...
            # 计算总的GAN损失
            gan_loss = errD_real + errD_rec_noise
            ...
            # 每2个batch打印一次损失信息
            if i % 2 == 0:
                print('[%d/%d][%d/%d]\tLoss_gan: %.4f\tLoss_decoder:%.4f\tLoss_prior: %.4f\tRec_loss: %.10f\tdis_real_loss: %0.4f\tdis_fake_loss: %.4f'
                      % (epoch,epochs, i, len(train_loader),
                         gan_loss.item(), de_loss.item(), prior_loss.item(),rec_loss.item(),errD_real.item(),errD_rec_noise.item()))
    ...
    # 绘制损失曲线
    plot_loss(prior_loss_list, 'models7_prior_loss0.png')  # 先验损失误差
    plot_loss(recon_loss_list, 'models7_recon_loss0.png')  # 重建损失误差
    plot_loss(gan_loss_list, 'models7_gan_loss0.png')   # GAN损失误差
